...
open class WSTagView: UIView, UITextInputTraits {
    ...
    public init(tag: WSTag) {
        ...
        closeButton.setImage(UIImage(named: "icon_close_red"), for: .normal)
        closeButton.addTarget(self, action: #selector(onDeleteClicked), for: .touchUpInside)
        self.displayText = tag.text.capitalizingFirstLetter()
        updateLabelText()
...
//        return CGSize(width: labelIntrinsicSize.width + layoutMargins.left + layoutMargins.right + 15,
...
//        notifyObservers(.vDeleteTag, data: textLabel.text)
    }
