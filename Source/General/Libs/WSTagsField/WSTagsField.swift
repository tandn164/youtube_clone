...
//            Util.showToast(message: String(format: "MSG_19".localized, tag.text), position: .top)
//            return
//        }
        else if self.tags.count >= maxTag {
            return
        }
        
        for item in self.tags {
            if item.text.lowercased() == tag.text.lowercased() {
                Util.showToast(message: String(format: "MSG_19".localized, tag.text), position: .top)
                return
            }
        }
        ...
        //Todo: update input textfield
        if self.tags.count >= maxTag {
...
extension WSTagsField {
    ...
    fileprivate func internalInit() {
        ...
        textField.layer.borderColor = AppResources.Color.fm_orange.cgColor
...
//        textField.spellCheckingType = .no
        ...
        textField.tintColor = AppResources.Color.fm_blue
    ...
    fileprivate func enumerateItemRects(layoutWidth: CGFloat, using closure: (_ tagView: WSTagView?, _ tagRect: CGRect?, _ textFieldRect: CGRect?) -> Void) {
        ...
        let availableWidthForTextField: CGFloat = maxWidth - curX

        if textField.isEnabled {
            var textFieldRect = CGRect.zero
            textFieldRect.size.height = Constants.STANDARD_ROW_HEIGHT

            let newSize = textField.sizeThatFits(CGSize(width: textField.frame.width,
                                                        height: Constants.STANDARD_ROW_HEIGHT))
            textFieldRect.size.width = max(125,newSize.width)
            if availableWidthForTextField < textFieldRect.size.width {
...
extension WSTagsField: UITextFieldDelegate {
    ...
    public func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        guard newString.length <= 20 else {
            return false
        }
